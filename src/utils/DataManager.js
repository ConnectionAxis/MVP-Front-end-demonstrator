...
export default class DataManager {
  ...
  static getObjects(collection, condition, limit, order) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (order) {
        if (order.length > 1) query = query.orderBy(order[0], order[1]);
        else query = query.orderBy(order[0]);
      }
  ...
  static updateCookieObject(name, data) {
    ...
    if (!Util.isEmpty(object)) {
      ...
      console.error(
        '[DataManager:updateCookieObject] No Cookie object "%s" found',
        name
      );
    }
  }
