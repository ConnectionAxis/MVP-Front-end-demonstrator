...
class App extends Component {
  ...
  handleUser(action) {
    switch (action) {
      case "logout":
        const user = DataManager.getCookieObject("conax-user");
        if (!Util.isEmpty(user)) DataManager.removeCookieObject("conax-user");
  ...
  defaultPath(e) {
    if (!Util.isEmpty(e.history)) this.history = e.history;

    if (this.state.session)
      return (
        <WorkSpace handleUser={this.handleUser} switchPath={this.switchPath} />
      );
    else {
