...
export default class CNavButton extends Component {
  constructor(props) {
    ...
    if (props.counter) this.cls += format(" {0}", "c-nav-btn-counter");

    if (props.hasOwnProperty("button")) this.isButton = props.button;
  }
  ...
  renderCNavButton(params) {
    ...
    if (params.counter) {
      output.push(
        <span key="counter" className="c-counter">
          {params.counter}
        </span>
      );
    }
    ...
    if (params.type === "user") {
      ...
      output.push(
        <span key="user" className="c-user-icon">
          {tx.split("")[0]}
        </span>
      );
    }
  ...
  render() {
    ...
    return (
      <>
        {this.isButton ? (
          <button className={this.cls} onClick={onClick}>
            {this.renderCNavButton(this.props)}
          </button>
        ) : (
          <div className={this.cls} onClick={onClick}>
            {this.renderCNavButton(this.props)}
          </div>
        )}
      </>
