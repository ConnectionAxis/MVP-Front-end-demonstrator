...
class PinCheckbox extends Component {
  constructor(props) {
    ...
    if (Util.isEmpty(this.props.label)) this.props.label = "PinCheckbox";

    if (!Util.isEmpty(this.props.className)) this.elCls = this.props.className;

    if (Util.isEmpty(this.props.id)) this.elId = Util.uniqueValue();
    else this.elId = this.props.id;
  ...
  render() {
    return (
      <div
        className={
          this.elCls +
          " pin-check d-flex justify-content-between align-items-center " +
          (this.state.check ? "checked" : "")
        }
      >
        <label className="c-link m-0 font-600" htmlFor={this.elId}>
          {this.props.label}
        </label>
