...
export default class UserProfile extends Component {
  ...
  componentDidMount() {
    ...
    if (!Util.isEmpty(this.props.id)) {
      DataManager.getObject("users", this.props.id).then(
        (data) => {
          console.log(data);
            ...
            <div className="container px-0">
              <div className="row no-gutters h-100">
                ...
                <div className="col-lg-7 col-md-6 col-sm-12 pt-3 pt-md-4 px-3 px-md-4">
                  <h1 className="h4 font-600 mb-2">
                    {t.firstname} {t.lastname}
                  </h1>
                  <p>{t.email}</p>
                </div>

                <div className="col-lg-3 col-md-4 pt-3 pt-md-4 pb-4 col-sm-12 px-3 px-md-0"></div>
              </div>
            </div>
          ) : (
            <>
