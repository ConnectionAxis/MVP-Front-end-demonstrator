...
export default class PopularFrameworks extends Component {
  ...
  loadData() {
    ...
    DataManager.getObjects("frameworks", [], 2, order).then(
      (data) => {
        ...
        {this.state.data.map((f, i) => (
          <div className="mb-4" key={i}>
            <div className="d-flex mt-1 mb-1">
              ...
              <div className="flex-fill pl-2">
                <h6 className="font-600">
                  <NavLink className="c-link" to={"/framework?id=" + f.id}>
                    {f.title}
                  </NavLink>
                </h6>
              </div>
            ...
            <p className="my-0 p-s">
              uploaded by{" "}
              <span className="c-link text-curious-blue">{f.author.name}</span>{" "}
              at {f.author.organization} - {this.getDate(f.created)}
            </p>
            <p className="my-0 p-s">
              at Research space -{" "}
              <NavLink className="c-link" to={"/research?id=" + f.research_ref}>
                {f.research}
              </NavLink>
            </p>
            <div className="mt-1">
                ...
                content={
                  <>
                    <p className="mb-2">
                      ...
                      {f.areas_research.split(",").map((r, i) => (
              ...
              >
                <span className="font-600 my-0 text-curious-blue c-link">
                  Research stack
                </span>
              </PopoverStickOnHover>
              <p className="font-600 my-0">
