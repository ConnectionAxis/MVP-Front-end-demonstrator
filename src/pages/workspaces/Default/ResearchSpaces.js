...
export default class ResearchSpaces extends Component {
  ...
  closeJumbotron() {
    ...
    if (!Util.isEmpty(user)) {
      DataManager.updateObject("users", user.id, { show_badge: false }).then(
        (userId) => {
  ...
  loadData() {
    DataManager.getObjects("research_topics", []).then(
      (data) => {
  ...
  topicAction(action, data) {
    switch (action) {
      case "report":
        this.setState({ showReportModal: true, report: data }, () => {
      ...
      <div className="px-md-4">
        {this.state.showbadge === true && (
          <div className="jumbotron mb-3 py-4 text-center position-relative">
            ...
            <p>
              A research space is interconnected framework that routes tools and
              activities from deferent research areas to build an end-to-end
              pipeline process on your project. Check out some already posted
              research spaces like the one below.
            </p>
            <button
            ...
            <Dropdown className="conax-nav-dropdown">
              ...
              <Dropdown.Menu alignRight className="mt-1">
                <Dropdown.Item
                  ...
                  href="#"
                  onClick={(e) => this.switchShow(e, "All")}
                >
                  All
                </Dropdown.Item>
                <Dropdown.Item
                  className="font-600 py-2 d-block"
                  href="#"
                  onClick={(e) => this.switchShow(e, "Related")}
                >
                  Related
                </Dropdown.Item>
              </Dropdown.Menu>
        ...
        <div className="research-spaces-list">
          {this.state.topics.map((t) => (
            <ResearchTopic key={t.id} data={t} onAction={this.topicAction} />
          ))}
        </div>
