    ...
    return (
      <div className="border px-3 pt-2 pb-3 mt-1 mb-3">
        <div className="d-flex align-items-start pt-1 pb-2">
          <div>{this.setAvatarPrimary(t.id)}</div>
          <div className="flex-fill pl-3">
            <h3 className="h5 font-600 mt-0 mb-2">
              <NavLink to={"/framework?id=" + t.id} className="c-link">
                {t.title}
              </NavLink>
            </h3>
            <p className="my-0">
              uploaded by{" "}
              <span className="c-link text-curious-blue">{t.author.name}</span>{" "}
              at {t.author.organization} - {this.getDate(t.created)}
            </p>
            <p className="my-0">
              at Research space -{" "}
              <NavLink className="c-link" to={"/research?id=" + t.research_ref}>
                {t.research}
              </NavLink>
            </p>
          </div>
        ...
        <div>
          ...
          <PopoverStickOnHover
            ...
            content={
              <>
                {t.areas_research.split(",").map((r, i) => (
          ...
          >
            <span className="text-curious-blue c-link mr-3">
              +{t.areas_research.split(",").length} Research areas
            </span>
          </PopoverStickOnHover>
          <PopoverStickOnHover
            placement="bottom"
            content={
              <>
                {t.areas_focus.split(",").map((r, i) => (
                  <p className="my-1" key={i}>
                    <a href="#areas_focus" className="text-curious-blue c-link">
                      {r.trim()}
                    </a>
                  </p>
                ))}
          ...
          >
            <span className="text-curious-blue c-link mr-3">
              +{t.areas_focus.split(",").length} Main focus areas
            </span>
          </PopoverStickOnHover>
          <PopoverStickOnHover
            placement="bottom"
          ...
          >
            <span className="text-curious-blue c-link">
              +{t.categories.split(",").length + t.topics.split(",").length}{" "}
              Categories & Topics
            </span>
          </PopoverStickOnHover>
        </div>
        <div>
          <span className="font-600 mr-2">Contribuitors:</span>+
          {t.contribuitors}{" "}
          <span className="text-curious-blue c-link">more</span>
        </div>
              ...
              >
                ...
                {t.inspired.map((inspire, i) => {
                  ...
                  return (
                    ...
                    >
                      This framework is inspired by your {collection} -{" "}
                      <NavLink
                        className="c-link"
